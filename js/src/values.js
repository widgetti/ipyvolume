...
class Values {
    constructor(names, names_vec3, getter, sequence_index, names_vec4) {
        ...
        this.merge_to_vec3 = (names, new_name) => {
        ...
        this.add_attributes = (geometry, postfix) => {
            ...
            _.each(this.array_vec3, (array, name) => {
                if (name.indexOf("selected") === -1) {
                    // selected attributes should not be send to the shader
                    let attr = new THREE.InstancedBufferAttribute(array, 3, false, 1);
            ...
            _.each(this.array_vec4, (array, name) => {
                if (name.indexOf("selected") === -1) {
                    // selected attributes should not be send to the shader
                    let attr = new THREE.InstancedBufferAttribute(array, 4, false, 1);
                    attr.normalized = name.indexOf("color") === -1 ? false : true; // color should be normalized
                    geometry.addAttribute(name + postfix, attr);
                }
            });
            _.each(this.scalar, (scalar, name) => {
                if (name.indexOf("selected") === -1) {
                    // selected attributes should not be send to the shader
                    let attr = new THREE.InstancedBufferAttribute(new Float32Array([scalar]), 1, false, this.length);
                    geometry.addAttribute(name + postfix, attr);
                }
            });
            _.each(this.scalar_vec3, (scalar_vec3, name) => {
                if (name.indexOf("selected") === -1) {
                    // selected attributes should not be send to the shader
                    let attr = new THREE.InstancedBufferAttribute(scalar_vec3, 3, false, this.length);
                    attr.normalized = name.indexOf("color") == -1 ? false : true; // color should be normalized
                    geometry.addAttribute(name + postfix, attr);
                }
            });
            _.each(this.scalar_vec4, (scalar_vec4, name) => {
                if (name.indexOf("selected") === -1) {
                    // selected attributes should not be send to the shader
                    let attr = new THREE.InstancedBufferAttribute(scalar_vec4, 4, false, this.length);
                    attr.normalized = name.indexOf("color") == -1 ? false : true; // color should be normalized
                    geometry.addAttribute(name + postfix, attr);
                }
            });
