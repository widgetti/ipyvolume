...
<script src="{rel_script_path}{fname_require}" crossorigin="anonymous" data-main='./{rel_script_path}' ></script>
<script>
    require.config({{
      map: {{
        '*': {{
...
{subsnippet}
        """.format(rel_script_path=rel_script_path, fname_fontawe=fname_fontawe, fname_require=fname_require,
                   fname_pyv=os.path.splitext(fname_pyv)[0], fname_three=os.path.splitext(fname_three)[0], subsnippet=subsnippet)
